...
Set the cache directory, the bind address if other hosts should reach the server, and (optionally) your Semantic Scholar API key. The cache directory is required: its default, `/data/scholar-mcp`, is the container path, and the unit's `ProtectSystem=strict` leaves only `/var/lib/scholar-mcp` writable.
...
See [Configuration](https://pvliesdonk.github.io/scholar-mcp/unstable/configuration/index.md) for all available variables.
...
1. Installs the matching version from PyPI
1. Reloads systemd
1. Restarts the service if it was running

Your configuration in `/etc/scholar-mcp/env` is preserved.

Check the bind address after upgrading from 2.0.0 or earlier

The unit file is replaced on every upgrade, and earlier versions set `SCHOLAR_MCP_HOST=0.0.0.0` in it. The unit no longer sets a bind address, so a service that was reachable from other hosts without naming the variable in `/etc/scholar-mcp/env` binds loopback after the post-install restart. Add `SCHOLAR_MCP_HOST=0.0.0.0` to that file to keep it reachable.
