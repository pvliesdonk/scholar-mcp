...
- **Background jobs**: slow PDF work returns a job handle to poll with `get_job_result`, while a fast call or a cache hit answers directly. The remaining rate-limited tools still use the older task queue and `get_task_result`.
