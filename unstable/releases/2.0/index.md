...
1. **Recreate your reverse-proxy routing yourself.** Copy the overlay from [Docker deployment](https://pvliesdonk.github.io/scholar-mcp/unstable/deployment/docker/index.md) into `compose.override.yml`, keep the `ports: !reset []` line, and check the result with `docker compose config` before starting anything.
