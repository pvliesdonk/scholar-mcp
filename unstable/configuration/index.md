...
| `SCHOLAR_MCP_INSTRUCTIONS_EXTRA` | *(none)*           | Operator context appended to the generated MCP instructions (deployment notes, house rules). The generated text (identity, tool workflows, documentation pointer) stays intact.                         |
...
| `SCHOLAR_MCP_BEARER_TOKENS_FILE`       | *(none)*      | Path to a TOML file mapping bearer tokens to per-caller subjects. When set, takes precedence over `SCHOLAR_MCP_BEARER_TOKEN`. See [Mapped bearer tokens](https://pvliesdonk.github.io/scholar-mcp/unstable/guides/authentication/#mapped-bearer-tokens-multi-subject) in the authentication guide.    |
...
See [Authentication guide](https://pvliesdonk.github.io/scholar-mcp/unstable/guides/authentication/index.md) for setup instructions and [OIDC deployment](https://pvliesdonk.github.io/scholar-mcp/unstable/deployment/oidc/index.md) for provider-specific configuration.
...
See [Docker deployment → Remote debugging](https://pvliesdonk.github.io/scholar-mcp/unstable/deployment/docker/#remote-debugging) for the IDE-attach walkthrough.
