...
The Required column below marks the oidc-proxy set. Setting all four selects that mode and omitting the two client credentials selects remote, so `SCHOLAR_MCP_AUTH_MODE` is the way to state the choice rather than leave it to be inferred. With none of these set, the server starts and serves unauthenticated. See the [authentication guide](https://pvliesdonk.github.io/scholar-mcp/unstable/guides/authentication/index.md) for setup, mapped multi-subject tokens, and troubleshooting.
