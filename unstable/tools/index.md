...
| `year_start`      | int          | *(none)*      | Filter: earliest publication year                                   |
| `year_end`        | int          | *(none)*      | Filter: latest publication year                                     |
| `fields_of_study` | list[string] | *(none)*      | S2 field-of-study names (such as `["Computer Science", "Physics"]`) |
| `venue`           | string       | *(none)*      | Filter by venue name                                                |
...
`limit` applies to cached and freshly fetched results alike. A cached author record holding fewer publications than `limit` is fetched again, unless it already holds every publication the author has.
...
| `offset`          | int          | `0`          | Pagination offset             |
| `year_start`      | int          | *(none)*     | Filter: earliest year         |
| `year_end`        | int          | *(none)*     | Filter: latest year           |
| `fields_of_study` | list[string] | *(none)*     | Field-of-study filter         |
| `min_citations`   | int          | *(none)*     | Minimum citation count filter |
...
See [Citation Graphs guide](https://pvliesdonk.github.io/scholar-mcp/unstable/guides/citation-graphs/index.md) for usage patterns.
...
| `negative_ids` | list[string] | *(none)*     | S2 paper IDs to steer recommendations away from |
...
A refused lookup is not a missing book
...
| Parameter            | Type   | Default         | Description                                                                                   |
| -------------------- | ------ | --------------- | --------------------------------------------------------------------------------------------- |
| `query`              | string | *(none)*        | Keyword search, searches patent titles and abstracts. Optional when using structured filters. |
| `cpc_classification` | string | *(none)*        | CPC classification code filter (such as `"H01M10/00"`)                                        |
| `applicant`          | string | *(none)*        | Applicant (assignee) name filter                                                              |
| `inventor`           | string | *(none)*        | Inventor name filter                                                                          |
| `date_from`          | string | *(none)*        | Earliest date (`YYYY-MM-DD`)                                                                  |
| `date_to`            | string | *(none)*        | Latest date (`YYYY-MM-DD`)                                                                    |
| `date_type`          | string | `"publication"` | Date field: `publication`, `filing`, or `priority`                                            |
| `jurisdiction`       | string | *(none)*        | Country code filter (such as `EP`, `US`, `WO`)                                                |
| `limit`              | int    | `10`            | Results per page (max 100)                                                                    |
| `offset`             | int    | `0`             | Pagination offset                                                                             |
...
| `text_offset`   | integer         | `0`          | Character offset at which the Markdown page starts                                                                 |
...
When `markdown` is present, the response also reports `text_offset`, `text_returned_chars`, `text_total_chars`, and `text_truncated`. `next_offset` is present when another page remains; pass it back as `text_offset`.
...
EPO serves patent images a single page per request, so a full document costs one request per page and is reassembled before conversion. A 21-page patent takes roughly 20 seconds to download; longer patents scale linearly, which is part of why this tool commonly answers with a job handle.
...
Scholar MCP supports Tier 1 standards bodies (NIST, IETF, W3C, ETSI) with full metadata and optional full-text conversion. Tier 2 paywalled bodies (ISO, IEC, IEEE) are tracked in [GitHub issues](https://github.com/pvliesdonk/scholar-mcp/issues).
...
When `full_text` is present, the response also reports `text_offset`, `text_returned_chars`, `text_total_chars`, and `text_truncated`. `next_offset` is present when another page remains; pass it back as `text_offset`.
...
A refused lookup is not a missing standard
...
Reports the last run of each Tier 2 standards sync. Returns one record per body, see `scholar-mcp sync-standards` in the CLI docs.

**Returns:** `{"runs": [{"body", "upstream_ref", "added", "updated", "unchanged", "withdrawn", "errors", "started_at", "finished_at"}]}`. An empty `runs` list means no sync has been run yet.
...
Converted Markdown is paged at 20,000 characters by default. Responses with Markdown include `text_offset`, `text_returned_chars`, `text_total_chars`, and `text_truncated`; `next_offset` points to the next page when one remains.
...
See [PDF Conversion guide](https://pvliesdonk.github.io/scholar-mcp/unstable/guides/pdf-conversion/index.md) for setup instructions.
...
| `filename`    | string          | *(none)*     | Filename stem for caching (such as `"smith2024_attention"`). Derived from the URL if omitted. |
| `use_vlm`     | bool            | `false`      | Enable VLM enrichment for formulas and figures                                                |
| `text_offset` | integer         | `0`          | Character offset at which the Markdown page starts                                            |
| `max_chars`   | integer or null | `20000`      | Page size, capped at 20,000; use `null` only when the client can accept the complete text     |
