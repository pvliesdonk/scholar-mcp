...
@pytest.fixture
def mcp_with_epo(bundle: ServiceBundle, epo_client: EpoClient, jobs: Jobs) -> FastMCP:
...
async def test_search_patents_uses_cache(
    bundle: ServiceBundle, epo_client: EpoClient, jobs: Jobs
) -> None:
...
async def test_get_patent_citations_section(bundle: ServiceBundle, jobs: Jobs) -> None:
...
async def test_npl_chapter_info_parsed(bundle: ServiceBundle, jobs: Jobs) -> None:
...
async def test_npl_no_chapter_info(bundle: ServiceBundle, jobs: Jobs) -> None:
...
def test_fetch_patent_pdf_no_epo_client(bundle: ServiceBundle, jobs: Jobs) -> None:
...
def test_fetch_patent_pdf_invalid_number(bundle: ServiceBundle, jobs: Jobs) -> None:
...
def test_fetch_patent_pdf_returns_result_inline(
    bundle: ServiceBundle, jobs: Jobs
) -> None:
    """fetch_patent_pdf answers inline when the download beats the deadline."""
    epo = _make_epo_client()
    ...
    assert Path(data["pdf_path"]).read_bytes() == b"%PDF-1.4 fake pdf content"
    epo.get_pdf.assert_called_once()
