...
!!! note
    The server sends Semantic Scholar requests at least 1.1 seconds apart.
    Without an API key, those requests share Semantic Scholar's public
    capacity and may be throttled.
...
An API key gives an introductory allowance of 1 request per second across
Semantic Scholar endpoints. It avoids reliance on the shared anonymous pool;
