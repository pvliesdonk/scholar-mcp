...
twice, and `convert_pdf_to_markdown` and `fetch_pdf_by_url` reuse an existing
markdown file rather than converting again. `fetch_and_convert` reuses the
PDF but always re-runs the conversion, so calling it twice for the same paper
converts twice (tracked in [#318](https://github.com/pvliesdonk/scholar-mcp/issues/318)).
