...
Two polling tools, because two mechanisms are in play. Each response names the
one to call, so follow `poll_with` rather than guessing.
...
Status values: `working`, `completed`, `failed`, `cancelled`. Unlike
`get_task_result`, `result` is a JSON object rather than a JSON string.
