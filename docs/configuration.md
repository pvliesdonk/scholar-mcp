...
Transport, identity, and tool visibility. `SCHOLAR_MCP_SERVER_NAME` and
`SCHOLAR_MCP_INSTRUCTIONS_EXTRA` let an operator rename an instance or
append deployment notes to the generated MCP instructions without touching
code; the legacy `SCHOLAR_MCP_INSTRUCTIONS` replaces the generated text
...
exclusive, and `SCHOLAR_MCP_AUTH_MODE` is normally inferred from which
...
<!-- Replace with operator-facing prose about this project's configuration:
     ...
     src/scholar_mcp/config.py — that generates the tables below; a
