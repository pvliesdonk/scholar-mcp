...
- **On HTTP 429**: the server holds all its S2 requests at one shared gate.
  Retries wait 1, 2, then 4 seconds; an exhausted retry ladder also delays the
  next request. A 429 can still reach the caller after the retry ladder; #409
  tracks background deferral.
