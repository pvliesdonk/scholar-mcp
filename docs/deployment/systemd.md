...
The unit binds `0.0.0.0` (all interfaces) via `Environment=SCHOLAR_MCP_HOST=0.0.0.0` so the packaged server is reachable from other hosts. The CLI's own fallback is `127.0.0.1` (loopback only) — to restrict the packaged service to loopback (for example behind a same-host reverse proxy), set `SCHOLAR_MCP_HOST=127.0.0.1` in `/etc/scholar-mcp/env`, which overrides the unit's default.
